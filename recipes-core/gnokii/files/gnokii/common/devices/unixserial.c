...
again:
	...
	FD_SET(fd, &readfds);
	if (state->sensorFd > 0) {
        FD_SET(state->sensorFd, &readfds);
		if (state->sensorFd > fd) maxFd = state->sensorFd;
	}

	int ret = select(maxFd + 1, &readfds, NULL, NULL, timeout);
	if (state->sensorFd > 0 && ret > 0 && FD_ISSET(state->sensorFd, &readfds)) {
		state->callbacks.on_sensor(state, NULL);
        goto again;
	}
	return ret;
}
